...
class TransMolecular(object):
    def __call__(self, sample, 
        ...
        res = torch.zeros((sample.shape[0], 5+self.chromosome_embedding_dim+self.gene_embedding_dim))
        ...
        for i in range(sample.shape[0]):
            ...
            curr_res = np.zeros(5+self.chromosome_embedding_dim+self.gene_embedding_dim)
            
            curr_res[0] = curr_sample[9]
            curr_res[1] = curr_sample[10]
            curr_res[2] = curr_sample[2]
            curr_res[3] = curr_sample[3]
            
            curr_effect = curr_sample[8]
            curr_res[4] = effects_survival_map.get(curr_effect)
            
            curr_chrom = chromosomes_map.get(curr_sample[1])
            curr_res[5:(5+self.chromosome_embedding_dim)] = chromosome_embeddings[curr_chrom]
            
            curr_gene = genes_map.get(curr_sample[6])
            curr_res[5+self.chromosome_embedding_dim:5+self.chromosome_embedding_dim+self.gene_embedding_dim] = gene_embeddings[curr_gene]
